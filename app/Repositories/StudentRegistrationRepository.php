...
class StudentRegistrationRepository
{
    ...
    public function search(array $filters): LengthAwarePaginator
    {
        return StudentRegistration::query()
            ...
            ->when($filters['document_status'] ?? null, fn ($query, string $status) => $query->where('passport_upload_status', $status))
            ->when($filters['verification_status'] ?? null, fn ($query, string $status) => $query->where('verification_status', $status))
            ->when($filters['receipt_status'] ?? null, fn ($query, string $status) => $query->whereHas('receiptRequests', fn ($receipt) => $receipt->where('status', $status)))
            ->when(isset($filters['needs_accommodations']) && $filters['needs_accommodations'] !== '', fn ($query) => $query->where('needs_accommodations', (bool) $filters['needs_accommodations']))
            ->when($filters['accommodation_status'] ?? null, fn ($query, string $status) => $query->where('accommodation_status', $status))
            ->when(isset($filters['preparation_interest']) && $filters['preparation_interest'] !== '', fn ($query) => $query->where('preparation_interest', (bool) $filters['preparation_interest']))
