...
class UpdateLandingContentRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'settings.copy.*.*' => ['required', 'string', 'max:1000'],
    ...
    protected function prepareForValidation(): void
    {
        ...
        foreach (['seo', 'hero', 'copy'] as $group) {
            foreach (($payload['settings'][$group] ?? []) as $key => $value) {
                $payload['settings'][$group][$key] = $this->normalizeLocalized($value);
            }
        }

        foreach (($payload['registration_settings'] ?? []) as $key => $value) {
            $payload['registration_settings'][$key] = $this->normalizeLocalized($value);
        }

        foreach (($payload['sections'] ?? []) as $key => $section) {
            foreach (['eyebrow', 'title', 'body', 'items'] as $field) {
                $payload['sections'][$key][$field] = $this->normalizeLocalized($section[$field] ?? '');
            }
        }

        $groups = [
            'timelines' => ['round', 'month', 'description'],
            'fees' => ['name', 'description'],
        ...
        foreach ($groups as $group => $fields) {
            foreach (($payload[$group] ?? []) as $index => $row) {
                foreach ($fields as $field) {
                    $payload[$group][$index][$field] = $this->normalizeLocalized($row[$field] ?? '');
                }
            }
        }

        foreach (['organization', 'office_hours', 'address', 'social_links'] as $field) {
            $payload['contact'][$field] = $this->normalizeLocalized($payload['contact'][$field] ?? '');
        }
    ...
    {
        if (! is_array($value)) {
            return ['en' => (string) $value, 'zh_TW' => (string) $value];
        }
        ...
        return [
            'en' => $english,
            'zh_TW' => (string) ($value['zh_TW'] ?? $english),
        ];
    }
}
