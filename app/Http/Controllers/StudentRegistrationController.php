        ...
        if ($step === 3) {
            $validator->after(function ($validator) use ($request): void {
                ...
                if ($options->count() !== count($practiceUuids)) {
                    $validator->errors()->add('practice_exams', 'One or more selected practice exams are no longer available.');
                    return;
                }
                ...
                if ($fullOption) {
                    $validator->errors()->add('practice_exams', "{$fullOption->name} has reached its seat capacity.");
                }
            });
        }
